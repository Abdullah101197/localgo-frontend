...
// Hook for live location broadcasting
const useLocationBroadcaster = (activeOrders) => {
  useEffect(() => {
      ...
      if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            // Update all currently picked up orders
            for (const order of pickupOrders) {
              try {
                await api.patch(`/orders/${order.id}/location`, {
                  latitude,
                  longitude,
                });
                console.log(`Updated location for Order #${order.id}`);
              } catch (err) {
                console.error(
                  `Failed to update location for Order #${order.id}`,
                  err,
                );
              }
            }
...
export default function RiderDashboard() {
  ...
  // Activate broadcaster for any orders in 'pickup' status
  useLocationBroadcaster(
    orders.filter(
      (o) => o.status === "pickup" && o.rider_id === user?.rider?.id,
    ),
  );
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (activeTab === "available") {
        ...
        filteredOrders = filteredOrders.filter(
          (o) => o.rider_id === user?.rider?.id && o.status !== "delivered",
        );
        ...
        <div className="flex flex-col w-full gap-4">
          <div className="flex gap-2">
            <a
              ...
              className="flex-1 bg-gray-100 text-gray-900 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-200 transition-all text-center flex items-center justify-center gap-2"
            >
            ...
            <button
              ...
              className="relative flex-1 bg-primary-50 text-primary-600 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-primary-600 hover:text-white transition-all text-center flex items-center justify-center gap-2"
            >
              <MessageSquare className="w-3.5 h-3.5" /> Chat
              {order.unread_messages_count > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-red-500 text-white text-[9px] font-black flex items-center justify-center rounded-full border-2 border-white animate-in zoom-in">
  ...
  const tabs = [
    {
      ...
      label: "Available",
      icon: Navigation,
      count: orders.length && activeTab === "available" ? orders.length : null,
    },
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 flex flex-col lg:flex-row">
      ...
      <main className="flex-1 overflow-x-hidden pt-6 pb-20 px-4 sm:px-8 lg:px-12">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            {orders.some((o) => o.status === "pickup") && (
              <div className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-xl shadow-lg shadow-primary-200 animate-pulse">
            ...
            {orders.length === 0 ? (
              <div className="col-span-full py-20 text-center bg-white rounded-[2.5rem] border border-gray-100 shadow-sm transition-all animate-in fade-in slide-in-from-bottom-5">
                ...
                <p className="text-gray-400 font-medium max-w-xs mx-auto">
                  Check back in a few minutes for new neighborhood deliveries.
                </p>
              </div>
            ) : (
              orders.map((order) => (
      ...
      <ChatOverlay
        ...
        recipientName={activeChatOrder?.customer?.name || "Customer"}
      />
    </div>
  );
}
